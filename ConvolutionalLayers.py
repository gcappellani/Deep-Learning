    ...
    def compute_errors(self):
        for i in range(len(self.errors)) :
            ...
            for j in range(self.next_layer.nkernel) :
                ...
                if len(loss_grad.shape) != len(self.next_layer.kernel_shape) :
                    loss_grad = to.add_axis(loss_grad, len(self.next_layer.kernel_shape) - len(loss_grad.shape))
...
class MaxPoolingLayer():
    def __init__(self, in_shape, kernel_shape, stride, operation,
        ...
        def compute_output(self, tensors):
            for i in range(len(tensors)):
                ...
                output = to.pooling(tensors[i], self.kernel, operation, self.stride)
